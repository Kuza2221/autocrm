...
router.get('/', verifyToken, (req, res) => {
  ...
  const clients = db.prepare(
    `SELECT id, name, phone, email FROM clients WHERE name LIKE ? OR phone LIKE ? OR email LIKE ? LIMIT 5`
  ).all(like, like, like);
  ...
  const vehicles = db.prepare(
    `SELECT v.id, v.brand, v.model, v.plate, v.vin, c.name as client_name, c.id as client_id
     ...
     WHERE v.plate LIKE ? OR v.vin LIKE ? OR v.brand LIKE ? OR v.model LIKE ? LIMIT 5`
  ...
  if (isNum) {
    orders = db.prepare(
      `SELECT o.id, o.status, o.complaint, o.created_at, c.name as client_name, v.brand, v.model, v.plate
       ...
       WHERE o.id = ? OR c.name LIKE ? OR v.plate LIKE ? ORDER BY o.created_at DESC LIMIT 5`
    ).all(parseInt(q), like, like);
  } else {
    orders = db.prepare(
      `SELECT o.id, o.status, o.complaint, o.created_at, c.name as client_name, v.brand, v.model, v.plate
       FROM orders o LEFT JOIN clients c ON c.id = o.client_id LEFT JOIN vehicles v ON v.id = o.vehicle_id
       WHERE c.name LIKE ? OR v.plate LIKE ? OR o.complaint LIKE ? ORDER BY o.created_at DESC LIMIT 5`
    ).all(like, like, like);
  }
