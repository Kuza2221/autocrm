...
function issueTokens(user, rememberMe, req, res) {
...
router.post('/', verifyToken, (req, res) => {
  ...
  try {
    const result = db.prepare('INSERT INTO users (name, email, password, role, company_id, email_verified) VALUES (?,?,?,?,?,1)').run(name, email, password || '', role, req.user.company_id);
...
// ── REGISTER worker (code + name + email, no password) ──────────────────────
router.post('/register-worker', (req, res) => {
  const { inviteCode, name, email, rememberMe = true } = req.body;
  if (!inviteCode || !name || !email) return res.status(400).json({ error: 'All fields required' });
  ...
  const nameExists = db.prepare('SELECT id FROM users WHERE LOWER(name)=LOWER(?) AND company_id=?').get(name.trim(), company.id);
  if (nameExists) return res.status(400).json({ error: 'Name already taken' });

  const emailLow = email.trim().toLowerCase();

  try {
    const result = db.prepare(
      'INSERT INTO users (name, email, password, role, company_id, email_verified) VALUES (?,?,?,?,?,1)'
    ).run(name.trim(), emailLow, '', 'mechanic', company.id);

    const user = db.prepare('SELECT id, name, email, role, company_id FROM users WHERE id=?').get(result.lastInsertRowid);
    return res.json(issueTokens(user, rememberMe, req, res));
  } catch(e) {
...
// ── LOGIN worker (code + name + email) ──────────────────────────────────────
router.post('/login-worker', (req, res) => {
  const { inviteCode, name, email, rememberMe = false } = req.body;
  if (!inviteCode || !name || !email) return res.status(400).json({ error: 'All fields required' });
  ...
  const user = db.prepare(
    'SELECT id, name, email, role, company_id FROM users WHERE LOWER(name)=LOWER(?) AND LOWER(email)=LOWER(?) AND company_id=?'
  ).get(name.trim(), email.trim().toLowerCase(), company.id);

  if (!user) return res.status(401).json({ error: 'Worker not found. Register first.' });

  return res.json(issueTokens(user, rememberMe, req, res));
});

// ── REGISTER owner (name + email + password + companyName) ──────────────────
router.post('/register', async (req, res) => {
  const { name, email, password, rememberMe = true, companyName } = req.body;
  ...
  try {
    const result = db.prepare(
      'INSERT INTO users (name, email, password, role, company_id, email_verified, verify_token) VALUES (?,?,?,?,?,0,?)'
    ).run(name, email.trim().toLowerCase(), password, 'admin', companyId, vToken);
    ...
    res.json({ pending: true, email, previewUrl, invite_code: code, company_name: cName });
  } catch(e) {
...
router.post('/resend-verification', async (req, res) => {
  ...
  const token = user.verify_token || crypto.randomBytes(32).toString('hex');
...
router.post('/login', (req, res) => {
  ...
  const user = db.prepare('SELECT id, name, email, role, company_id, email_verified FROM users WHERE LOWER(email)=LOWER(?) AND password=?').get(email?.trim(), password);
